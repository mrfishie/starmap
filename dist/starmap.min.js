...
function Model(t,e,i,o,s){EventEmitter.call(this),this.io=t,this.name=e,this.url=utils.resolveName(e),this.itemDef=i||{},this.modelDef=o||{},this.filter=s||{},this.children=[],this.eventHandlers={},this.bindings=[],this.value=publicModel(this,new Promise(function(t,e){this.on("ready",t())}.bind(this))),this.on("created",this._onCreated.bind(this)),this.on("updated",this._onUpdated.bind(this)),this.on("destroyed",this._onDestroyed.bind(this)),t.socket.on(e,function(t){this.emit(t.verb,t)}.bind(this)),this.refresh().then(function(){this.emit("ready")}.bind(this))}var Promise=require("bluebird"),_=require("lodash"),utils=require("./utils"),EventEmitter=require("events").EventEmitter,util=require("util"),publicModel=require("./publicModel"),modelItem=require("./modelItem");util.inherits(Model,EventEmitter),module.exports=Model,Model.prototype._addItem=function(t){-1===this.value.indexOf(t)&&this.value._push(t)},Model.prototype._removeItem=function(t){var e=this.value.indexOf(t);-1!==e&&this.value._splice(e,1)},Model.prototype._removeAll=function(){for(;this.value.length;)this.value._pop()},Model.prototype._onCreated=function(t){var e=modelItem(t.data,this);this.childMatches(e).then(function(t){t&&this._addItem(e)}.bind(this))},Model.prototype._onUpdated=function(t){var e=this.value.itemById(t.id);e&&e.update(t.data,!1)},Model.prototype._onDestroyed=function(t){var e=this.value.itemById(t.id);e&&this._removeItem(e)},Model.prototype.childMatches=function(t){return Promise.resolve(function(){return t.model.name!==this.name?!1:_.isEqual(this.filter,{})?!0:t.matches(this.filter)}.bind(this)())},Model.prototype.refresh=function(){return utils.socketGet(this.io,this.url,this.filter).then(function(t){var e=_.isArray(t)?t:[t];this._removeAll();for(var i=0;i<e.length;i++)this._addItem(modelItem(e[i],this))}.bind(this))},Model.prototype.save=function(){_.forEach(this.value,function(t){t.save()})};

},{"./modelItem":4,"./publicModel":5,"./utils":6,"bluebird":7,"events":8,"lodash":13,"util":12}],2:[function(require,module,exports){
var _=require("lodash"),Model=require("./Model"),utils=require("./utils"),modelCache={},isSubscribed={};module.exports=function(e,r,i,l,s){s=s||{},r=r.toLowerCase();var u=r+JSON.stringify(s);if(_.has(modelCache,u)){var o=modelCache[u];return i&&(o.itemDef=i,o.refresh()),l&&(o.modelDef=l),o.value}var a=new Model(e,r,i,l,s);return isSubscribed[r]||(utils.socketGet(e,a.url),isSubscribed[r]=!0),modelCache[u]=a,a.value};

},{"./Model":1,"./utils":6,"lodash":13}],3:[function(require,module,exports){
function get(e){function r(r,t,n){return gm(e,r,t,n)}return r.model=r,r.create=function(e,t){return r(e).create(t)},r.find=function(e,t){return r(e).find(t)},r.findOne=function(e,t){return r(e).findOne(t)},r.calculate=utils.createCalculateProperty,r}var Promise=require("bluebird"),_=require("lodash"),gm=require("./getModel"),utils=require("./utils");window.io?module.exports=get(window.io):module.exports=get,window.starmap=module.exports;
},{"./getModel":2,"./utils":6,"bluebird":7,"lodash":13}],4:[function(require,module,exports){
function update(e,r,t,n){var u=[],i=[];return _.forEach(t,function(r,t){_.isFunction(r)?u.push(Promise.resolve(r()).then(function(r){e[t]=r})):e[t]=r,i.push(r)}),_.forEach(r.itemDef,function(r,o){if(r&&r.__precalc){var s=r.__precalc,a=t[o];u.push(Promise.resolve(s.call(e,a,n||{})).then(function(r){e[o]=r&&r._stWrapped?r.val:r}))}else _.has(e,o)&&-1===i.indexOf(o)||(e[o]=r)}),Promise.all(u)}function serverProperties(e,r,t){var n={},u=[];return _.forEach(t||e,function(e,t){null!=e&&(e._isModelItem?(n[t]=e.id,u.push(e)):_.isArray(e)?n[t]=_.map(e,function(e){return e._isModelItem?(u.push(e),e.id):e}):_.has(r.itemDef,t)?_.has(r.itemDef[t],"__connection")&&(n[t]=e):n[t]=e)}),{properties:n,refresh:u}}function create(e,r,t){var n=serverProperties(e,r,t).properties;return utils.socketPut(r.io,r.url+"/create/",n).then(function(t){return _.merge(n,t),update(e,r,n)}).then(function(){return Promise.all(_.map(n.refresh,function(e){return e.refresh()}))})}function modelItem(e,r){var t={};return utils.defineProperties(t,{_isModelItem:{value:!0},model:{value:r.value},update:{value:function(e,n){return null==n&&(n=!0),update(t,r,e).then(function(){return n?t.save():void 0})}},save:{value:function(){if(_.has(t,"id")){var e=serverProperties(t,r);return utils.socketPost(r.io,r.url+"/update/"+t.id,e.properties).then(function(){return Promise.all(_.map(e.refresh,function(e){return e.refresh()}))})}return create(t,r)}},refresh:{value:function(){return _.has(t,"id")?utils.socketGet(r.io,r.url+"/"+t.id).then(function(e){return update(t,r,e,{noRefresh:!0})}):create(t,r)}},"delete":{value:function(){return r._removeItem(t),utils.socketDelete(r.io,r.url,{id:t.id})}},matches:{value:function(e){return utils.socketGet(r.io,r.url,e).then(function(e){for(var r=0;r<e.length;r++)if(e[r].id===t.id)return!0;return!1})}},then:{value:function(){return t.ready.then.apply(t.ready,arguments)}},"catch":{value:function(){return t.ready["catch"].apply(t.ready,arguments)}}}),t.ready=Promise.resolve(),update(t,r,e||{}),t}var _=require("lodash"),utils=require("./utils"),Promise=require("bluebird");module.exports=modelItem;

},{"./utils":6,"bluebird":7,"lodash":13}],5:[function(require,module,exports){
function publicModel(e,r){function t(r){var t=_.merge({},e.filter,r),a=getModel(e.io,e.name,e.itemDef,e.modelDef,t);return-1===e.children.indexOf(a)&&e.children.push(a),a}function a(e){for(var r=[],t=0;t<n.length;t++)-1!==e.indexOf(n[t].id)&&r.push(n[t]);return r}var n=[];return utils.defineProperties(n,{_isModel:{value:!0},name:{value:e.name},ready:{value:r},create:{value:function(r){r=r||{};var t=modelItem(r,e);return t.ready=t.update(r).then(function(){e._addItem(t)}),t}},itemById:{value:function(e){for(var r=0;r<this.length;r++)if(this[r].id===e)return this[r];return!1}},find:{value:t},filter:{value:t},findOne:{value:function(e){return t(e).then(function(e){return e.length?e[0]:!1})}},refresh:{value:function(){return Promise.all(_.map(this,function(e){return e.refresh()}))}},then:{value:function(){return r.then.apply(r,arguments)}},"catch":{value:function(){return r["catch"].apply(r,arguments)}},_pop:{value:arrayPop},pop:{value:function(){var e=arrayPop.call(this);return e["delete"](),e}},_push:{value:arrayPush},push:{value:function(){for(var e=0;e<arguments.length;e++)this.create(arguments[e]);return this.length}},_shift:{value:arrayShift},shift:{value:function(){var e=arrayShift.call(this);return e["delete"](),e}},_splice:{value:arraySplice},splice:{value:function(r,t){var a=_.map(arraySlice.call(arguments,2),function(r){var t=modelItem(r,e);return t.ready=t.update(r),t});a.unshift(r,t);for(var n=arraySplice.apply(this,a),u=0;u<n.length;u++)n["delete"]();return n}},_unshift:{value:arrayUnshift},unshift:{value:function(){return arrayUnshift.apply(this,_.map(arguments,function(r){var t=modelItem(r,e);return t.ready=t.update(r),t}))}},__connection:{value:e.name}}),_.merge(n,utils.createCalculateProperty(function(e,t){if(e){var n,u=_.isArray(e);return n=u?_.map(e,function(e){return _.isNumber(e)?e:e.id}):[_.isNumber(e)?e:e.id],r.then(function(){function e(){return u?{_stWrapped:!0,val:r}:{_stWrapped:!0,val:r.length?r[0]:null}}var r=a(n);return t.noRefresh?e():Promise.all(_.map(r,function(e){return e.refresh()})).then(e)})}}.bind(this))),n}var Promise=require("bluebird"),_=require("lodash"),utils=require("./utils"),getModel=require("./getModel"),modelItem=require("./modelItem"),arrayPop=Array.prototype.pop,arrayPush=Array.prototype.push,arrayShift=Array.prototype.shift,arraySplice=Array.prototype.splice,arrayUnshift=Array.prototype.unshift,arraySlice=Array.prototype.slice;module.exports=publicModel;

},{"./getModel":2,"./modelItem":4,"./utils":6,"bluebird":7,"lodash":13}],6:[function(require,module,exports){
function promisifySocket(e){return function(r,t,o){requestId++;return new Promise(function(n,s){r.socket[e](t,o,function(e){e.error?s(e.error):_.isString(e)?s(new Error(e)):n(e)})})}}var _=require("lodash");exports.resolveName=function(e){var r=e.indexOf("/"),t="";return-1!==r&&(t=e.substring(0,r)+"/",e=e.substring(r+1)),"/"+t+e};var requestId=0;exports.socketGet=promisifySocket("get"),exports.socketPost=promisifySocket("post"),exports.socketPut=promisifySocket("put"),exports.socketDelete=promisifySocket("delete"),exports.defineProperties=function(){return Object.defineProperties?Object.defineProperties:function(e,r){_.extend(e,_.mapValues(r,function(e){return e.value}))}}(),exports.createCalculateProperty=function(e,r){return _.merge({__precalc:e},r)};
